...
@router.get("/me", response_model=schemas.full.UserFull)
async def read_users_me(current_user: User = Depends(get_current_user), db: Session = Depends(dep_dbsession)):
    """
    ...
    """
    return quick_retrieve(db, User, id=current_user.id)


@router.post("/", response_model=schemas.read.UserRead)
async def create_user(user: schemas.edit.UserNew, db: Session = Depends(dep_dbsession)):
    """
    ...
    return quick_create(db, User(name=user.name, surname=user.surname, password=h, email=user.email))


@router.put("/{user_id}", response_model=schemas.read.UserRead)
async def edit_user(edits: schemas.edit.UserNew, user_id: UUID,
    ...
    """
    intermediate_model = schemas.edit.UserEdit(name=edits.name, surname=edits.surname,
                                               password=bcrypt.hashpw(bytes(edits.password, encoding="utf-8"),
                                                                      bcrypt.gensalt()),
                                               email=edits.email)
    target = quick_retrieve(db, User, id=user_id)
